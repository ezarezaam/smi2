...
const InvoiceManagement: React.FC = () => {
  ...
  const fetchInvoices = async () => {
    try {
      ...
      toast.error('Terjadi kesalahan saat memuat data invoice');
    }
  };
  ...
  const fetchAvailableSOs = async () => {
    try {
      ...
      // Filter SO yang sudah delivered dan belum fully invoiced
      const availableSOs = (data || []).filter(so => 
        so.delivery_status !== 'pending' && so.invoice_status !== 'fully_invoiced'
      );
  ...
  const handleSOSelect = async (soId: string) => {
    if (!soId) {
      setSelectedSO(null);
      setFormData(prev => ({ ...prev, sales_order_id: '', customer_id: '' }));
      return;
    }
    ...
    try {
      ...
      if (data) {
        ...
        const deliveredItems = (data.items || []).filter(item => (item.delivered_quantity || 0) > 0);
        const subtotal = deliveredItems.reduce((sum, item) => 
          sum + ((item.delivered_quantity || 0) * item.unit_price), 0
        );
    ...
    const submitInvoice = async () => {
      try {
        ...
        if (selectedInvoice?.id) {
          const { error } = await InvoiceService.update(selectedInvoice.id, formData);
          if (error) throw error;
          toast.success('Invoice berhasil diperbarui');
        } else {
          const { error } = await InvoiceService.createFromSalesOrder(
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setFormData({...formData, sales_order_id: e.target.value});
                      if (e.target.value) {
                        handleSOSelect(e.target.value);
                      }
                    }}
                    className="w-full border border-gray-300 rounded-md px-3 py-2"
                  >
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full border border-gray-300 rounded-md px-3 py-2"
                    readOnly={!!formData.sales_order_id}
                  />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
