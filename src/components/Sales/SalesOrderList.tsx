...
import { Download, Calendar, TrendingUp, PieChart, BarChart3, FileText, Save } from '../icons';
import { FinancialReport, REPORT_TYPES } from '../../models/GeneralLedger';
...
const FinancialReports: React.FC = () => {
  ...
  const loadSavedReports = async () => {
    try {
      const { data, error } = await GeneralLedgerService.getFinancialReports();
      if (error) throw error;
      setSavedReports(data || []);
    } catch (error: any) {
      toast.error('Gagal memuat laporan tersimpan: ' + error.message);
    }
  };
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      switch (selectedReport) {
        case 'profit-loss':
          result = await GeneralLedgerService.generateProfitLoss(dateFrom, dateTo);
          break;
        case 'balance-sheet':
          result = await GeneralLedgerService.generateBalanceSheet(dateTo);
          break;
        case 'cash-flow':
          result = await GeneralLedgerService.generateCashFlow(dateFrom, dateTo);
          break;
        case 'aging':
          result = await GeneralLedgerService.generateAgingReport(dateTo);
          break;
        default:
      ...
      toast.error('Gagal generate laporan: ' + error.message);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const saveReport = async () => {
    ...
    try {
      const reportName = `${REPORT_TYPES[selectedReport as keyof typeof REPORT_TYPES]} - ${formatDate(dateFrom)} to ${formatDate(dateTo)}`;
      
      const { error } = await GeneralLedgerService.saveFinancialReport({
  ...
  const reportTypes = [
    ...
    { id: 'balance-sheet', name: 'Neraca', icon: BarChart3 },
    { id: 'cash-flow', name: 'Arus Kas', icon: Calendar },
    { id: 'aging', name: 'Aging Report', icon: PieChart }
  ];
