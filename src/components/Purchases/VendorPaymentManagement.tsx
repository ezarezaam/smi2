...
import { VendorPayment, VENDOR_PAYMENT_STATUS, VENDOR_PAYMENT_METHODS } from '../../models/VendorPayment';
...
const VendorPaymentManagement: React.FC = () => {
  ...
  const [formData, setFormData] = useState<VendorPayment>({
    payment_number: '',
    vendor_id: '',
    bill_id: '',
  ...
  const loadPayments = async () => {
    try {
      const { data, error } = await VendorPaymentService.getAll();
  ...
  const loadOutstandingBills = async () => {
    try {
      ...
      if (error) throw error;
      // Filter bills yang belum lunas
      const outstanding = (data || []).filter(bill => bill.payment_status !== 'paid');
  ...
  const handleAdd = () => {
    setFormData({
      payment_number: VendorPaymentService.generatePaymentNumber(),
      vendor_id: '',
      bill_id: '',
      payment_date: new Date().toISOString().split('T')[0],
      amount: 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedPayment?.id) {
        const { error } = await VendorPaymentService.update(selectedPayment.id, formData);
        if (error) throw error;
        toast.success('Pembayaran berhasil diperbarui');
      } else {
        const { error } = await VendorPaymentService.create(formData);
        if (error) throw error;
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-2xl font-bold">Vendor Payment Management</h1>
        <button
          onClick={handleAdd}
        ...
        <div className="bg-white rounded-lg shadow p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900">{outstandingBills.length}</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-lg shadow p-6">
      ...
      <div className="bg-white rounded-lg shadow p-4">
        <div className="flex space-x-4">
          ...
          >
            ...
            {Object.entries(VENDOR_PAYMENT_STATUS).map(([key, label]) => (
          ...
          <tbody className="divide-y divide-gray-200">
            {filteredPayments.map((payment) => (
              <tr key={payment.id} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {VENDOR_PAYMENT_METHODS[payment.payment_method as keyof typeof VENDOR_PAYMENT_METHODS]}
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <span className={`px-2 py-1 text-xs rounded-full ${getStatusColor(payment.status)}`}>
                    {VENDOR_PAYMENT_STATUS[payment.status as keyof typeof VENDOR_PAYMENT_STATUS]}
                  </span>
                </td>
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    {outstandingBills.map(bill => (
                      <option key={bill.id} value={bill.id}>
                        {bill.bill_number} - {formatCurrency(bill.total_amount - (bill.paid_amount || 0))} remaining
                      </option>
                    ))}
                  </select>
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full border border-gray-300 rounded-md px-3 py-2"
                  >
                    {Object.entries(VENDOR_PAYMENT_METHODS).map(([key, label]) => (
                      <option key={key} value={key}>{label}</option>
                    ))}
                  </select>
                </div>
