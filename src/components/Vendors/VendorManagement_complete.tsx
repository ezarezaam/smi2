...
const VendorManagement: React.FC = () => {
  ...
  const fetchVendors = async () => {
    ...
    try {
      ...
      if (error) {
        toast.error('Gagal memuat data vendor: ' + (error.message || 'Unknown error'));
        return;
      }
      if (data) {
        // Filter hanya contacts dengan type vendor atau both
  ...
  const handleAdd = async () => {
    try {
      ...
      if (error) {
        toast.error('Gagal menambahkan vendor: ' + (error.message || 'Unknown error'));
        return;
      }
      if (data) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (error) {
        toast.error('Gagal mengupdate vendor: ' + (error.message || 'Unknown error'));
        return;
      }
      if (data) {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (error) {
        toast.error('Gagal menghapus vendor: ' + (error.message || 'Unknown error'));
        return;
      }
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData({...formData, type: e.target.value as 'vendor' | 'customer' | 'both'})}
                  className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                >
