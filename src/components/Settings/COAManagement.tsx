...
const COAManagement: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Validasi panjang kode COA (maksimal 8 karakter)
      if (formData.code && formData.code.length > 8) {
        toast.error('Kode COA maksimal 8 karakter');
      ...
      if (editingId) {
        ...
        if (error) {
          ...
          if (err.code === '22001') {
            toast.error('Kode COA terlalu panjang, maksimal 8 karakter');
            return;
          }
          throw error;
        }
        ...
        if (error) {
          ...
          if (err.code === '22001') {
            toast.error('Kode COA terlalu panjang, maksimal 8 karakter');
            return;
          } else if (err.code === '23505' && err.message?.includes('coa_pkey')) {
